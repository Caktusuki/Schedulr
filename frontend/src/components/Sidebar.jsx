...
export default function Sidebar({ showSidebar }) {
  ...
  ${toggleSidebar ? "px-3 md:justify-start w-full" : "px-1 md:justify-center w-fit"} py-1 sm:py-2 transition-all duration-300 rounded-md hover:bg-gray-300 dark:hover:bg-gray-800 ${isActive ? "bg-gray-300 dark:bg-gray-800" : ""
    }`;
    ...
    >
      ...
      {user && (
        <div className={`${toggleSidebar ? "block" : "hidden"} my-6 p-3 bg-gray-200 dark:bg-gray-800 rounded-lg`}>
          <p className="text-sm text-gray-700 dark:text-gray-300">Logged in as:</p>
      ...
      {/* Controls: collapse sidebar + theme toggle */}
      <div className="absolute bottom-20 left-0 w-full flex items-center justify-center gap-3 px-2">
        <button
          onClick={() => setToggleSidebar(!toggleSidebar)}
          className="hidden md:inline bg-gray-200 dark:bg-gray-900 p-2 rounded-full"
...
function ThemeToggle() {
  ...
  return (
    <button
      onClick={toggle}
      className="bg-gray-200 dark:bg-gray-800 text-sm px-3 py-2 rounded-full hover:bg-gray-300 dark:hover:bg-gray-700 transition-colors"
