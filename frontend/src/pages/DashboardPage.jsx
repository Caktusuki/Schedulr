    ...
    <div className="space-y-8">
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-40 overflow-y-auto">
              {dailyTasks.slice(0, 6).map((task) => (
                <div
                  key={task.id}
                  className={`p-3 rounded-lg border cursor-pointer transition-all ${task.isCompleted
                    ? 'bg-green-50 dark:bg-green-900 border-green-200'
                    : 'bg-gray-50 dark:bg-gray-900 dark:border-gray-700 border-gray-200 hover:border-gray-300'
                    }`}
                  onClick={() => toggleDailyTaskCompletion(task.id)}
                >
                  <div className="flex items-center space-x-2">
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${task.isCompleted
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-medium truncate ${task.isCompleted ? 'line-through text-gray-500 dark:text-gray-300' : 'text-gray-900 dark:text-gray-100'
                        }`}>
                        {task.name}
                      </p>
          ...
          {todayTasks.length === 0 ? (
            ...
            <div className="space-y-3">
              {todayTasks.slice(0, 3).map((task) => (
                <div key={task.id} className="flex flex-wrap items-center gap-2 justify-between p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
                  <div>
                    <p className="font-medium text-gray-800">{task.name}</p>
                    <p className="text-xs sm:text-sm text-gray-500">{task.priority} priority</p>
                  </div>
                  <div className={`px-2 py-1 rounded text-xs font-medium ${task.status === 'completed' ? 'bg-green-100 text-green-800' :
            ...
            <div className="space-y-3">
              {upcomingTasks.map((task) => (
                <div key={task.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
                  ...
                  <div className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full ${task.priority === 'high' ? 'bg-red-500' :
      ...
      <div className="bg-white dark:bg-gray-800 dark:border dark:border-gray-700 rounded-lg shadow p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <button
            onClick={() => navigate('/tasks')}
            className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-left dark:text-gray-100"
          >
